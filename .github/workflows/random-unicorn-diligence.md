...
1. Pick one eligible target unicorn.
  ...
  - Pick one candidate at random (`shuf -n 1`); if research later fails eligibility, replace that company with another uncovered eligible candidate.

2. Research and write the report.
  - For the selected company, run the skill at [.agents/skills/startup-diligence-research/SKILL.md](.agents/skills/startup-diligence-research/SKILL.md) end-to-end at standard depth.
  - Output file: `reports/<slug>-diligence-report.yaml`.
  - Keep intermediate work minimal and avoid creating unnecessary companion artifacts.
...
3. Open the draft PR.
  - Open a draft PR via `create-pull-request`:
    ...
    - Title: `[unicorn-diligence] Add diligence report for 1 unicorn`
    - Body: short summary table with company, sector, country, latest valuation, and top 3 risks.
    - Files: only `reports/<slug>-diligence-report.yaml` for the selected company (companion artifacts allowed only under `reports/` with the same matching `<slug>` prefix).

## Rules

- Keep exactly one target company per run.
- If no usable unicorn list can be fetched, or no uncovered eligible candidates remain, create a `missing-tool` issue and exit.
